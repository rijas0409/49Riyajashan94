...
interface VetData {
  ...
  medicalSpecializations?: Record<string, { primary: string; secondary: string[] }>;
    ...
    const fetchVetProfile = async () => {
      ...
      try {
        ...
        setDoctor({
          ...
          clinicalExpertise: vetProfile.clinical_expertise || [],
          medicalSpecializations: (vetProfile.medical_specializations as Record<string, { primary: string; secondary: string[] }>) || {},
          emergencyAvailable: vetProfile.emergency_available || false,
          support24x7: vetProfile.support_24x7 || "no",
          weekendAvailability: vetProfile.weekend_availability || "no",
        });
      } catch (err) {
    ...
    <div className="min-h-screen bg-background flex flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto pb-28">
        ...
        <div className="px-4 mt-5 space-y-5">
          ...
          {/* Medical Specialties */}
          {doctor.medicalSpecializations && Object.keys(doctor.medicalSpecializations).length > 0 && (
            <section className="bg-white border border-slate-100 rounded-2xl p-4 shadow-xs space-y-3.5">
              <h2 className="text-sm font-extrabold text-foreground tracking-tight uppercase flex items-center gap-1.5 text-purple-950 border-b border-slate-50 pb-1.5">
                <Stethoscope className="w-4 h-4 text-purple-600" />
                Medical Specialties & Focus
              </h2>
              <div className="space-y-3 divide-y divide-slate-50">
                {Object.entries(doctor.medicalSpecializations).map(([species, data]: [string, any]) => {
                  if (!data || !data.primary) return null;
                  return (
                    <div key={species} className="pt-2.5 first:pt-0 space-y-1.5">
                      <span className="px-2.5 py-0.5 rounded bg-pink-50 text-pink-700 text-[10px] font-black uppercase tracking-wider">{species}s</span>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          <p className="text-[10px] text-slate-400 font-semibold uppercase">Primary Specialty</p>
                          <p className="font-extrabold text-slate-800">{data.primary}</p>
                        </div>
                        {data.secondary && data.secondary.length > 0 && (
                          <div>
                            <p className="text-[10px] text-slate-400 font-semibold uppercase">Secondary Focus</p>
                            <p className="font-semibold text-slate-600 leading-relaxed">{data.secondary.join(", ")}</p>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {doctor.clinicalExpertise && doctor.clinicalExpertise.length > 0 && (
            <section className="bg-white border border-slate-100 rounded-2xl p-4 shadow-xs space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5 pt-0.5">
                {doctor.clinicalExpertise.map((tag) => (
                  <span key={tag} className="bg-purple-50 text-purple-700 border border-purple-100/50 text-[11px] font-extrabold px-3 py-1 rounded-full shadow-2xs">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          {/* Consultation Formats, Prices, and Emergency Care */}
          <section className="bg-white border border-slate-100 rounded-2xl p-4 shadow-xs space-y-4">
            <h2 className="text-sm font-extrabold text-purple-950 tracking-tight uppercase flex items-center gap-1.5 border-b border-slate-50 pb-1.5">
              <Clock className="w-4 h-4 text-purple-600" />
              Consultation Formats & Pricing
            </h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {(() => {
                const lower = (doctor.consultationType || "").toLowerCase();
                const showOnline = lower.includes("video") || lower.includes("both") || lower.includes("consultation");
                const showInClinic = lower.includes("clinic") || lower.includes("both") || lower.includes("visit");
                const showHome = lower.includes("home") || lower.includes("both") || lower.includes("visits");

                return (
                  <>
                    {(showOnline) && (
                      <div className="flex items-center justify-between p-3 rounded-xl border border-indigo-50 bg-indigo-50/10">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600">
                            <Video className="w-4 h-4" />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-slate-800">Video Consultation</p>
                            <p className="text-[10px] text-slate-400">Standard online video</p>
                          </div>
                        </div>
                        <span className="text-xs font-black text-slate-800">₹{doctor.onlineFee}</span>
                      </div>
                    )}
                    {showInClinic && (
                      <div className="flex items-center justify-between p-3 rounded-xl border border-purple-50 bg-purple-50/10">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-purple-50 flex items-center justify-center text-purple-600">
                            <Briefcase className="w-4 h-4" />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-slate-800">In-Clinic Visit</p>
                            <p className="text-[10px] text-slate-400">At practitioner clinic</p>
                          </div>
                        </div>
                        <span className="text-xs font-black text-slate-800">₹{doctor.onlineFee}</span>
                      </div>
                    ...
                    {showHome && (
                      <div className="flex items-center justify-between p-3 rounded-xl border border-pink-50 bg-pink-50/10">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-pink-50 flex items-center justify-center text-[#EC4899]">
                            <Home className="w-4 h-4" />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-slate-800">Home Visit</p>
                            <p className="text-[10px] text-slate-400">At your pet's home</p>
                          </div>
                        </div>
                        <span className="text-xs font-black text-slate-800">₹{doctor.offlineFee}</span>
                      </div>
                    )}
                  </>
                );
            ...
            <div className="grid grid-cols-2 gap-3 pt-1 text-xs">
              <div className="flex items-center gap-2 bg-[#FAFCFA] border border-emerald-100 rounded-xl p-2.5">
                <div className={`w-2 h-2 rounded-full ${doctor.emergencyAvailable ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`} />
                <div>
                  <p className="font-extrabold text-slate-800">Emergency Case</p>
                  <p className="text-[10px] text-slate-400">{doctor.emergencyAvailable ? "Available" : "Not Accepting"}</p>
                </div>
              </div>

              <div className="flex items-center gap-2 bg-[#FAFAFC] border border-slate-100 rounded-xl p-2.5">
                <Clock className="w-4 h-4 text-purple-600" />
                <div>
                  <p className="font-extrabold text-slate-800">24/7 Hours</p>
                  <p className="text-[10px] text-slate-400">{doctor.support24x7 === "yes" ? "Fully Open" : "Standard Hours"}</p>
                </div>
              </div>
            </div>
          </section>
